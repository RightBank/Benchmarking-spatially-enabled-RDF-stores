...
private ArrayList<SPARQLQuerySetMember> querySet = new ArrayList<SPARQLQuerySetMember>();
	...
	public SPARQLQueryGeographicaSet() {
		...
		SPARQLQuerySetMember Q1 = new SPARQLQuerySetMember("Q1:Boundary_CLC",queryPrefix,			
				" SELECT (geof:boundary(?o1) AS ?result) " + 
				"  WHERE { " + 
				"  		?s1 clc:asWKT ?o1. " + 
				" } "); 
				
		SPARQLQuerySetMember Q2 = new SPARQLQuerySetMember("Q2:Envelope_CLC",queryPrefix,	
				" SELECT (geof:envelope(?o1) AS ?result) " +
		        " WHERE { " + 
			    " 		?s1 clc:asWKT ?o1. " +
		        " } "); 

		SPARQLQuerySetMember Q3 = new SPARQLQuerySetMember("Q3:ConvexHull_CLC",queryPrefix,	
				" SELECT (geof:convexHull(?o1) AS ?result) " +
		        " WHERE { " +
			    " 		?s1 clc:asWKT ?o1. " + 
		        " } "); 

		SPARQLQuerySetMember Q4 = new SPARQLQuerySetMember("Q4:Buffer_GeoNames_2",queryPrefix,	
		        " SELECT (geof:buffer(?o1, 4, opengis:metre) AS ?result) " +
		        " WHERE { " +                                                                           
		        " 		?s1 geonames:asWKT ?o1. " +
		        " } ");

		SPARQLQuerySetMember Q5 = new SPARQLQuerySetMember("Q5:Buffer_LGD_2",queryPrefix,	
				" SELECT (geof:buffer(?o1, 4, opengis:metre) AS ?result) " + 
		        "  WHERE { " +
			    "  		?s1 lgd:asWKT ?o1. " +
		        " } "); 
		...
		SPARQLQuerySetMember Q11 = new SPARQLQuerySetMember("Q11:Overlaps_CLC_GivenPolygon",queryPrefix,
			    " SELECT ?result " + 
			    "  WHERE { " +
				"  			?s1 clc:asWKT ?result. " +
		...
		SPARQLQuerySetMember Q12 = new SPARQLQuerySetMember("Q12:Crosses_LGD_GivenLine",queryPrefix,
			    " SELECT ?result "  +
			    "  WHERE { " + 
				"  			?s1 lgd:asWKT ?result. " +
		...
		SPARQLQuerySetMember Q13 = new SPARQLQuerySetMember("Q13:Within_GeoNames_GivenPolygon",queryPrefix,
				" SELECT ?result " +
			    "  WHERE { " + 
		...
		SPARQLQuerySetMember Q14 = new SPARQLQuerySetMember("Q14:Within_GeoNames_Point_Buffer",queryPrefix,
				" SELECT ?result " + 
			    "  WHERE { " + 
				"  		 		?s1 geonames:asWKT ?result. " + 
				"  FILTER(geof:sfWithin(?result, geof:buffer(\"" + fixedPoint + "\"^^geo:wktLiteral , 3000, opengis:metre))). " +  
			    " } "); 
		...
		SPARQLQuerySetMember Q22 = new SPARQLQuerySetMember("Q22:Within_GeoNames_GAG",queryPrefix,
				"	SELECT ?result " +
				"	 WHERE { " + 
				"			?result geonames:asWKT ?o1. " + 
				"           ?s2 gag:asWKT ?o2. " +  
		...
		SPARQLQuerySetMember Q23 = new SPARQLQuerySetMember("Q23:Within_LGD_GAG",queryPrefix,
				"	SELECT ?result " +
				"	 WHERE { " + 
				"			?result lgd:asWKT ?o1. " + 
				"           ?s2 gag:asWKT ?o2. " +  
		...
		SPARQLQuerySetMember Q24 = new SPARQLQuerySetMember("Q24:Within_CLC_GAG",queryPrefix,
				"	SELECT ?result " +
				"	 WHERE { " + 
				"			?result clc:asWKT ?o1. " + 
				"           ?s2 gag:asWKT ?o2. " +  
		...
		SPARQLQuerySetMember Q25 = new SPARQLQuerySetMember("Q25:Crosses_LGD_GAG",queryPrefix,
				"	SELECT ?result " +
				"	 WHERE { " + 
				"			?result lgd:asWKT ?o1. " + 
				"           ?s2 gag:asWKT ?o2. " +  
		...
		SPARQLQuerySetMember Q26 = new SPARQLQuerySetMember("Q26:Touches_GAG_GAG",queryPrefix,
				"	SELECT ?result " +
				"	 WHERE { " + 
				"			?result gag:asWKT ?o1. " + 
				"           ?s2 gag:asWKT ?o2. " +  
		...
		SPARQLQuerySetMember Q27 = new SPARQLQuerySetMember("Q27:Overlaps_GAG_CLC",queryPrefix,
				"	SELECT ?result " +
				"	 WHERE { " + 
				"			?result gag:asWKT ?o1. " + 
				"           ?s2 clc:asWKT ?o2. " +  
